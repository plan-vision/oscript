...
/**
 * A symbol table implementation based on a open hash map, using double hashing
 * as a strategy to avoid collisions. Double hasing uses two hash functions to
 ...
 * <p>
 * Threading note: this class is not synchronized, but is designed to save to
 * read from multiple threads, while write from a single thread context (at a
 * time).
 * 
 * @author Rob Clark (rob@ti.com)
 * @version 1.0
 */
public class OpenHashSymbolTable implements SymbolTable {
	/**
	...
	/**
	 * The state is maintained in an inner class, so it can be automically updated
	 * when we have to resize the the tables.
	 */
	private State state;
	...
	private class State {
		...
		/**
		 * Once the <code>size</code> exceeds the threshold, it is time to grow the
		 * table and re-hash.
		 */
		public final int threshold;

		/**
		 * A key value of <code>0</code> in the <code>keys</code> indicates an unused
		 * slot.
		 */
		...
		State(int capacity) {
			if (capacity <= 3) 
				capacity = 3;	// result of checkCapacity(3)
			else 
	 ...
	 * Class Constructor.
	 * 
	 * @param capacity the initial capacity (MIN 2)
	 ...
	 * @return an index, or <code>-1</code> if no mapping exists for the specified
	 *         symbol
	 */
	...
	/**
	 * Get the index that the specified symbol maps to, and create a new one if a
	 * mapping does not already exist. If a new mapping is created, it's value is
	 * the next successive array index, ie. the the previous array index plus one.
	 * The first mapping created has the value zero.
	 * 
	 * @param id the id of the symbol to get a mapping for
	 * @return an index
	 */
	...
	private static final int putIfNotIn(State state, int id, int val) {
		...
		throw new ProgrammingErrorException("shouldn't get here if load factor is less than one!!");
	}

	/**
	 * Some statistics gathering code... all calls to the stat gathering methods
	 * should be wrapped with an if(COLLECT_STATS) so it gets compiled out for a
	 * regular build.
	 */
	...
	/**
	 * Return an iteration of the keys (symbols) into this table. To conform to the
	 * {@link java.util.Iterator} interface, each symbol is wrapped (boxed) in a
	 * {@link Integer}.
	 * 
	...
	/*
	 * The hash functions... for now I'm using endianess swap as the primary hash
	 * function (which should deal well with symbol ids from 0 to n... ideal would
	 * probably be a bitwise reverse, but I think this is good enough
	 * 
	 * These hashing functions rely on the table size being a prime number. Using an
	 * alternate version of h2() could result in this restriction being lifted, but
	 * I am not sure which approach performs better, so some benchmarking is
	 * probably in order...
	 */
	private static final int h1(int k, int capacity) {
		int v = (((k & 0xff000000) >> 24) | ((k & 0x00ff0000) >> 8) | ((k & 0x0000ff00) << 8)
				| ((k & 0x000000ff) << 24));
