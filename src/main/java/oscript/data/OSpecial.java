...
/**
 * An <code>OSpecial</code> is used for different special values that aren't
 * really members of any other type. The values of Special are unique, and
 * special care is taken when constructing, and serializing and unserializing
 * the special to ensure that this property is preservied... ie. there are never
 * two instances of <code>NULL</code>.
 * 
...
public final class OSpecial extends Value {
	...
	protected OSpecial(String str) {
		if (specials.get(str) != null)
			throw new ProgrammingErrorException("duplicate special!");
		this.str = str;
	}
	...
	protected PackagedScriptObjectException noSuchMember(String member) {
		return PackagedScriptObjectException.makeExceptionWrapper(new ONoSuchMemberException(getType(), member) // SOME
																												// MORE
																												// SPECIFIC
																												// INFORMATION
	...
	protected PackagedScriptObjectException noSuchMember(Value type, String member) {
		return PackagedScriptObjectException.makeExceptionWrapper(new ONoSuchMemberException(type, member)
		// new ONullReferenceException(str)
		);
	}
	...
	public static final FunctionValueWrapper functionValueWrapperTrue = new FunctionValueWrapper(
			OBoolean.makeBoolean(true));
	public static final FunctionValueWrapper functionValueWrapperFalse = new FunctionValueWrapper(
			OBoolean.makeBoolean(false));
