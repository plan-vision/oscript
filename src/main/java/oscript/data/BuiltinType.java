 ...
 * @version 1.38
 */
public class BuiltinType extends JavaClassWrapper
{
  /**
  ...
  /**
   * The parameter types to the <code>javaClassConstructor</code>
   */
  ...
  private static final Class getJavaClass( String name )
  {
    try {
      return forName(name);
  ...
  protected synchronized void init()
  {
    if( !initialized )
    {
      ...
      // JavaClassWrapper initializes itself only when needed, so we need
      ...
      {
        // ignore... cannot be called as constructor
      }
