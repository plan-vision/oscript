...
public class JavaObjectWrapper extends Value
{
    ...
    throws PackagedScriptObjectException
  {
    ...
    if( (javaObject instanceof Comparable) &&
        (valJavaObject instanceof Comparable) )
    {
    ...
    throws PackagedScriptObjectException
  {
    if( javaObject instanceof List )
    {
      ...
      Value arr = new OArray();
      
      for( int i=i1; i<=i2; i++ )
        arr.elementAt( JavaBridge.convertToScriptObject(i) ).
          opAssign( JavaBridge.convertToScriptObject( l.get(i) ) );
