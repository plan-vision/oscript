...
public class JavaClassWrapper extends Type {
	...
	/**
	 * Table of all java-class-wrappers. A <code>JavaClassWrapper</code> instance is
	 * intern'd, so there shouldn't be two instances representing the same java
	 * class.
	 */
	...
	public final static String[] MEMBER_NAMES = new String[] { "isA", "castToString", "castToJavaObject",
			"callAsConstructor", "callAsExtends", "getMember", "getName" };
	 ...
	 * right... otherwise you might have multiple wrappers per java class (ie type),
	 * which would confuse type checking...
	 * 
	...
	/* ======================================================================= */
	/**
	 * If this object is a type, determine if an instance of this type is an
	 * instance of the specified type, ie. if this is <code>type</code>, or a
	 * subclass.
	 * 
	...
	public Value callAsExtends(StackFrame sf, Scope scope, MemberTable args) throws PackagedScriptObjectException {
		throw PackagedScriptObjectException
				.makeExceptionWrapper(new OUnsupportedOperationException("java class is final; can't call as extend"));
	 ...
	 * @param exception whether an exception should be thrown if the member object
	 *                  is not resolved
	...
	/* ======================================================================= */
	/**
	 * Get a member of this type. This is used to interface to the java method of
	 * having members be attributes of a type. Regular object- script object's
	 * members are attributes of the object, but in the case of java types
	 * (including built-in types), the members are attributes of the type.
	 * 
	...
	protected Value getTypeMember(Value obj, int id) {
		...
		/*
		 * XXX hack: (needed because ExactNumber, InexactNumber, etc. cast to Long,
		 * Double, etc)
		 */
		if ((this instanceof BuiltinType) && !(obj instanceof ScriptObject))
			javaObj = obj;
		else
		...
		/*
		 * XXX hack: (kinda) if we don't find the method, perhaps it is provided by
		 * Value... this is sorta like multiple inheritance, but we need to ensure that
		 * even java objects implement certain basic methods... if an object is a
		 * ScriptObject, this is handled by the ScriptObject class... except getMember()
		 * won't return the method... so it will work for stuff like (foo instanceof
		 * Foo), but not foo.bopInstanceOf(Foo)
		 */
		if (val == null) {
			if (obj instanceof ScriptObject)
				val = ScriptObject.TYPE.getTypeMemberImpl(obj,
						Symbol.getSymbol("_" + Symbol.getSymbol(id).castToString()).getId());
			else
				val = Value.TYPE.getTypeMemberImpl(obj, id);
		}

		/*
		 * If we still haven't found it, check if it is a static member of the class:
		 */
		if (val == null)
	...
	/* ======================================================================= */
	/**
	 * Implements the reflection stuff... this is broken out into it's own class
	 * because a <code>JavaClassWrapper</code> might have two of these (which get
	 * initialized at different times), one for the regular class and one for the
	 * wrapper class. The wrapper class is only used when script code extends a java
	 * type. Also, since we need to create a new instance of
	 * <code>JavaInnerClassWrapper</code> for each access, we cache the impl's for
	 * inner classes, to avoid having to go thru the expensive init() process
	 * multiple times for the same java class.
	 */
	public static class JavaClassWrapperImpl {
		Class javaClass;
		 ...
		 * Class Constructor
		 */
		public JavaClassWrapperImpl(Class javaClass) {
			this.javaClass = javaClass;
		}
		...
		void init() {
			if (!initialized) {
				...
				Field[] fields = javaClass.getDeclaredFields(); // 15.10.2025 > switch to DeclaredFilds + check public,
																// fixes TeaVM classlib bug (TODO REMOVE AFTER FIX
																// approved)
				//-----------------------------------------------------------------------------------------------------
		...
		/**
		 * basically we decide on the constructor with the closest matching args, and
		 * call it to create a new instance.
		 */
		...
		public CacheEntry getInnerClass(int id) {
			try {
		...
		/**
		 * Check for "getter" and "setter" methods corresponding to <code>id</code>, and
		 * if present return a bean-accessor, to allow access to properties of java
		 * beans.
		 */
		private CacheEntry getBeanAccessor(int id) {
			...
			return new CacheEntry() {
				...
				public Value getMember(final int id, final Object javaObj) {
					/*
					 * work-around to deal with the case of this getting called while a script class
					 * that extends a java class is being constructed (but is not yet linked to the
					 * java object). Since no script types have bean properties to access, this is
					 * the easy work-around: <p> XXX Note work-around for work-around... I need to
					 * clean this up!!
					 */
	...
	private static final JavaBridge.JavaCallableAccessor constructorAccessor = new JavaBridge.JavaCallableAccessor() {
	...
	public static String getInnerClassName(Class javaClass, int id) {
	...
	private static class FieldCacheEntry implements CacheEntry {
		...
		void add(Field field) {
			...
			if ((this.field == null) || this.field.getDeclaringClass().isAssignableFrom(field.getDeclaringClass()))
				this.field = field;
		}
	...
	private static class MethodCacheEntry implements CacheEntry {
		...
		public void add(Method method) {
			...
			/*
			 * I'm not sure if we are supposed to be seeing duplicate methods, or not, but I
			 * am (at least under JDK v1.3.1 macosx... I haven't tested others), so try to
			 * deal with this in as sane a mannar as possibly by ignoring overriden methods:
			 */
	 ...
	 */
	/*
	 * private static final Value getMemberFromCache( SymbolMap memberCache, int id,
	 * Object obj ) { CacheEntry ce = (CacheEntry)(memberCache.get(id));
	 * 
	 * if( ce != null ) return ce.getMember( id, obj );
	 * 
	 * return null; }
	 */
	...
	/**
	 * add a method member to the specified cache, or if a member already exists for
	 * a method with the same name, append this method to the existing member
	 */
	...
	/* ======================================================================= */
	/**
	 * Derived classes that implement {@link #getMember} should also implement this.
	 * 
	 * @param s        the set to populate
	 * @param debugger <code>true</code> if being used by debugger, in which case
	 *                 both public and private/protected field names should be
	 *                 returned
	 * @see #getMember
	 */
	...
	/* ======================================================================= */
	/**
	 * Derived classes that implement {@link #getTypeMember} should also implement
	 * this.
	 * 
	 * @param s        the set to populate
	 * @param debugger <code>true</code> if being used by debugger, in which case
