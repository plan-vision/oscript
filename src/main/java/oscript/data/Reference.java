...
/**
 * A reference forwards all method calls to the object it is a reference for,
 * but additionally a reference is assignable. It is used when a variable (or
 * array element) is looked up within a scope, and can be used as either an lval
 * or rval.
 * <p>
 * When a property of an object (ie a scope) has attributes associated with it,
 * those attributes are attributes of the reference. The attributes are
 * partially enforced by this class (in the case of "const"), and by
 * <code>ScriptObject</code> (in the case of "public").
 * <p>
 * Note that this could extends {@link AbstractReference}, but doesn't because I
 * think that would have a performance impact... one of these days I should
...
public class Reference extends Value {
	...
	/**
	 * The attribute bit-mask, either {@link #ATTR_INVALID} or a mask of the other
	 * attribute constants.
	 */
	private int attr;
	...
	/* ======================================================================= */
	/**
	 * Reset this reference, which is used when the reference is re-used (in- stead
	 * of re-allocating)
	 */
	...
	/* ======================================================================= */
	/**
	 * For references to an object (ie variables), this returns the actual value
	 * this is a reference to, otherwise this return <code>this</code>.
	 * 
	...
	/* ======================================================================= */
	/**
	 * Get the type of this object. A reference doesn't actually have a type, but
	 * instead is the type of whatever it contains... really I am not sure if a
	 * reference is a first class type, or perhaps could be implemented as an
	 * inner-class for OArray and ScriptObject. Perhaps Value should be an
	 * interface, and what is now Value becomes some sort of adapter class?
	 * 
	 * @return the object's type
	 */
	...
	/* ======================================================================= */
	/**
	 * Perform the cast operation, <code>(a)b</code> is equivalent to
	 * <code>a.bopCast(b)</code>
	 * 
	...
	public void opAssign(Value val) throws PackagedScriptObjectException {
		...
		if (val == UNDEFINED)
			throw PackagedScriptObjectException
					.makeExceptionWrapper(new OUnsupportedOperationException("cannot assign (undefined) to variable"));
		// in the case of "const" values, they can be assigned a value
		// only once:
		if ((attr & ATTR_CONST) != 0) {
			if (readonly)
				throw PackagedScriptObjectException
						.makeExceptionWrapper(new OUnsupportedOperationException("cannot assign value to constant"));
			else
				readonly = true;
		}
